...
d19 = read.csv(file = 'uniqueEntities_2019_cleaned.csv', sep = ';', fill=TRUE)
...
#gpe words
gp = gp[-1,]
pal2 = c('#6495ED', '#1E90FF', '#20B2AA')

wordcloud(words=gp$entity, freq = gp$sum_frequency, size = 1.5, min.freq = 2,
          max.words = 25, scale = c(3, 0.6), random.order = FALSE, rot.per = 0,
          colors = pal2)

#norp words
pal73 = c('#8FBC8F', '#2E8B57', '#20B2AA')

wordcloud(words=np$entity, freq = np$sum_frequency, size = 1.5, min.freq = 2,
          max.words = 25, scale = c(3, 0.6), random.order = FALSE, rot.per = 0,
          colors = pal3)

#org words
pal4 = c( '#DDA0DD','#9370DB','	#9932CC', '#20B2AA')

wordcloud(words=op$entity, freq = op$sum_frequency, size = 1.5, min.freq = 2,
          max.words = 20, scale = c(4, 0.6), random.order = FALSE, rot.per = 0,
          colors = pal4)

#pers words
pal5 = c('#DB7093','#FFC0CB', '#B0C4DE', '#20B2AA')

wordcloud(words=pp$entity, freq = pp$sum_frequency, size = 1.5, min.freq = 2,
          max.words = 25, scale = c(4, 0.6), random.order = FALSE, rot.per = 0,
          colors = pal5)
...
pivot = pivot [-1,]
pal = c('#8FBC8F', '#2E8B57', '#20B2AA')

#plots the simple word cloud
wordcloud(words=pivot$entity, freq = pivot$sum_frequency, size = 1.5, min.freq = 2,
          max.words = 25, scale = c(3, 0.6), random.order = FALSE, rot.per = 0,
          colors = pal)
...
### 2.4) Location Map for Visualisation
...
# retrieve map with municipality borders from PDOK
gemeentegrenzen = st_read("https://geodata.nationaalgeoregister.nl/cbsgebiedsindelingen/wfs?request=GetFeature&service=WFS&version=2.0.0&typeName=cbs_gemeente_2021_gegeneraliseerd&outputFormat=json")
gemeentegrenzen = gemeentegrenzen %>%
  rename(c(gemeentecode = statcode, gemeentenaam = statnaam)) %>%
  mutate_if(is.character, str_to_lower)

# retrieve information on all woonplaatsen in 2021
alle_woonplaatsen = read_xlsx("alle_woonplaatsen.xlsx")
alle_woonplaatsen = alle_woonplaatsen %>%
  rename(woonplaatscode = statcode) %>%
  mutate_if(is.character, str_to_lower)

# retrieve information on all municipalities in 2021
alle_gemeenten = read_xlsx("alle_gemeenten.xlsx")
alle_gemeenten = alle_gemeenten %>%
  select(c(-Woonplaatscode_1, -Naam_4, -Code_5, -Naam_6, -Code_7)) %>%
  rename(c(woonplaatscode = Woonplaatsen, gemeentenaam = Naam_2, gemeentecode = Code_3)) %>%
  mutate_if(is.character, str_to_lower)

# retrieve relevant information in anbis_clean
anbis_info_gemeenten = subset(anbis_clean, select=("city"))
anbis_info_gemeenten = anbis_info_gemeenten %>%
  drop_na() %>%
  rename(gemeentenaam = city) %>%
  mutate_if(is.character, str_to_lower)
anbis_info_gemeenten$gemeentenaam = gsub("'", "", anbis_info_gemeenten$gemeentenaam)

# count how often particular cities occur
anbis_per_location = anbis_info_gemeenten %>%
  group_by(gemeentenaam) %>%
  summarise(count = n()) %>%
  arrange(desc(count))
anbis_per_location$gemeentenaam[anbis_per_location$gemeentenaam=="nieuwerkerk ad ijssel"] = "nieuwerkerk aan den ijssel" (this is me fixing all the municipalities that were spelled wrong)

# link the info in per_location to the info in alle_gemeenten
anbis_link_df = full_join(alle_woonplaatsen, anbis_per_location, by = "gemeentenaam")

# nog even de info van alle woonplaatsen linken aan gemeentes
anbis_link_df_2 = full_join(alle_gemeenten, anbis_link_df, by = "woonplaatscode")
anbis_link_df_2 = anbis_link_df_2 %>%
  select(-gemeentenaam.y) %>%
  rename(gemeentenaam = gemeentenaam.x)

# match the data in link_df to gemeentegrenzen
anbis_new_df = left_join(anbis_link_df_2, gemeentegrenzen, by = "gemeentecode")
anbis_new_df = anbis_new_df %>%
  select(c(-ID, -woonplaatscode, -gemeentenaam.y)) %>%
  rename(gemeentenaam = gemeentenaam.x)
...
anbis_new_df %>%
  ...
  labs(title = "Statutaire vestigingsplaats van ANBIs", fill = "") +
  theme_void()
